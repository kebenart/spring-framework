...
public abstract class AbstractAutoProxyCreator extends ProxyProcessorSupport
	...
	protected Object wrapIfNecessary(Object bean, String beanName, Object cacheKey) {
		// 如果当前bean已经被代理过了,直接返回代理bean
		if (StringUtils.hasLength(beanName) && this.targetSourcedBeans.contains(beanName)) {
			return bean;
		}
		// 判断是否是基础的bean,(继承(Advices、Advisors 和 AopInfrastructureBeans等)的bean不需要被代理)
		...
		// 获取aop
		// 获取当前bean所有的advice,advisor,interceptor
		Object[] specificInterceptors = getAdvicesAndAdvisorsForBean(bean.getClass(), beanName, null);
		// 判断是否需要创建代理对象
		if (specificInterceptors != DO_NOT_PROXY) {
			this.advisedBeans.put(cacheKey, Boolean.TRUE);
			// SingletonTargetSource 封装了真实实现类信息
			// 创建代理
	 ...
	 *	<p>默认实现将 Advices、Advisors 和 AopInfrastructureBeans 视为基础结构类。
	 ...
	 * @param beanClass the class of the bean
	 * @param beanName the name of the bean
	 * @param specificInterceptors 特定于此bean的一组拦截器(可能是空的，但不是空的)
	...
	protected Object createProxy(Class<?> beanClass, @Nullable String beanName,
		...
		//配置 proxy-target-class="true",这样不管有没有接口，都使用 CGLIB 来生成代理：
	...
	private Advisor[] resolveInterceptorNames() {
		...
		for (String beanName : this.interceptorNames) {
			if (cbf == null || !cbf.isCurrentlyInCreation(beanName)) {
				...
				// 增强器Advisor,它是Advice(通知)和Pointcut(切入点)的适配器
				// 添加增强器,同时在wrap函数中检测bean是否是advisor
