...
public abstract class FrameworkServlet extends HttpServletBean implements ApplicationContextAware {
	...
	protected WebApplicationContext initWebApplicationContext() {
		...
		// 获取根上下文,保存在ServletCOntext中,这个根上下文作为当前MVC上下文的双亲上下文
			...
			if (wac instanceof ConfigurableWebApplicationContext) {
				...
				if (!cwac.isActive()) {
					...
					if (cwac.getParent() == null) {
						// 上下文实例在没有显式父类的情况下被注入-> 设置根应用程序上下文(如果有的话;可能为空)作为父类
						cwac.setParent(rootContext);
					}
					// 配置并refresh WebApplicationContext
		...
		if (wac == null) {
			...
			// 如果存在，则假定已经设置了父上下文(如果有)，并且用户已经执行了任何初始化，例如设置上下文id
		...
		if (!this.refreshEventReceived) {
			...
			// 要么上下文不是一个支持刷新的ConfigurableApplicationContext，要么在构建时注入的上下文已经被刷新了——这里>手动触发初始onRefresh。
