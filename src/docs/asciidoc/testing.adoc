...
configuration for Spring MVC, use the <<spring-mvc-test-framework,_Spring MVC test.Test
Framework_>> instead.
====
...
test.Test classes typically declare either an array of __resource locations__ for XML or Groovy
...
----
	@WebAppConfiguration
	@ContextHierarchy({
		@ContextConfiguration(classes = test.AppConfig.class),
...
test.Test property sources have higher precedence than those loaded from the operating
...
combines the `@test.Test` and `@Tag` annotations from JUnit Jupiter with the `@Transactional`
...
it is responsible for. test.Test instances do not automatically receive access to the
...
----
	@RunWith(SpringRunner.class)
	// ApplicationContext will be loaded from "/test.AppConfig.groovy" and
	// "/TestConfig.groovy" in the root of the classpath
	**@ContextConfiguration({"/test.AppConfig.groovy", "/TestConfig.Groovy"})**
	public class MyTest {
		// class body...
	}
----
...
----
	@RunWith(SpringRunner.class)
	// ApplicationContext will be loaded from test.AppConfig and TestConfig
	**@ContextConfiguration(classes = {test.AppConfig.class, TestConfig.class})**
	public class MyTest {
		// class body...
	}
----
...
test.Test properties files can be configured via the `locations` or `value` attribute of
...
class hierarchy will be run within a transaction. test.Test methods that are not annotated
...
`@AfterAll`, `@BeforeEach`, `@AfterEach`, `@test.Test`, `@RepeatedTest`, `@ParameterizedTest`,
etc.
...
The __Spring MVC test.Test framework__ provides first class support for testing Spring MVC
...
JUnit Jupiter based example of using Spring MVC test.Test:
...
To be certain, none of the options in Spring MVC test.Test fall under the category of classic
...
===== Further Server-Side test.Test Examples
...
sample tests] intended to demonstrate how to use Spring MVC test.Test. Browse these examples
for further ideas. Also the
https://github.com/spring-projects/spring-mvc-showcase[spring-mvc-showcase] has full test
coverage based on Spring MVC test.Test.
...
With Spring MVC test.Test, we can easily test if we are able to create a `Message`.
...
* https://mockito.github.io[Mockito]: Java mock library based on the
  http://xunitpatterns.com/test.Test%20Spy.html[test spy] pattern.
