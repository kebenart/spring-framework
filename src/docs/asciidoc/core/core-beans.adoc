...
The `test.AppConfig` class above would be equivalent to the following Spring `<beans/>` XML:
...
----
	public static void main(String[] args) {
		ApplicationContext ctx = new AnnotationConfigApplicationContext(test.AppConfig.class);
...
----
	public static void main(String[] args) {
		...
		ctx.register(test.AppConfig.class, OtherConfig.class);
...
assuming that `test.AppConfig` is declared within the `com.acme` package (or any package
...
----
	<web-app>
		...
		<context-param>
			...
			<param-value>com.acme.test.AppConfig</param-value>
		</context-param>
...
container. In this example, we create a `@Configuration` class named `test.AppConfig` and
...
`@Configuration` annotation and process the `@Bean` methods declared in `test.AppConfig`
properly.
...
----
	<beans>
		...
		<bean class="com.acme.test.AppConfig"/>
...
[NOTE]
====
In `system-test-config.xml` above, the `test.AppConfig` `<bean/>` does not declare an `id`
...
----
	<beans>
		<!-- picks up and registers test.AppConfig as a bean definition -->
		<context:component-scan base-package="com.acme"/>
		<context:property-placeholder location="classpath:/com/acme/jdbc.properties"/>
