 ...
 *
 * 使用步骤
 * 1. 通过继承自AbstractBeanDefinitionReader里的方法,来使用ResourceLoader将资源文件路径转换为对应的Resource文件
 * 2. 通过DoucumentLoader对Resource进行转换,将Resource文件转换为Document文件
...
public class XmlBeanDefinitionReader extends AbstractBeanDefinitionReader {
	...
	protected int doLoadBeanDefinitions(InputSource inputSource, Resource resource)
			throws BeanDefinitionStoreException {
		try {
			// 将xml文件转为Document对象.这个过程是由documentLoader完成的
			Document doc = doLoadDocument(inputSource, resource);

			// 重点核心
			// 这里启动的是对BeanDefinitions解析的详细过程,这个解析过程会使用Spring的Bean配置规则
	...
	/**
	 * 注册给定DOM文档中包含的bean定义
	 * 由loadBeanDefinitions调用
	 * 创建解析器类的新实例，并在其上调用registerBeanDefinitions
	 *
