...
public class DefaultListableBeanFactory extends AbstractAutowireCapableBeanFactory
	...
	/** 对BeanDefinition的持有,key为beanName */
	...
	public void preInstantiateSingletons() throws BeansException {
		...
		for (String beanName : beanNames) {
			...
			// 非抽象,非懒加载的是singleton(单例)的进行初始化
			if (!bd.isAbstract() && bd.isSingleton() && !bd.isLazyInit()) {

				// 判断Bean是否是FactoryBean
				if (isFactoryBean(beanName)) {

					// FactoryBean在BeanName前面加"&",再调用getBean
	...
	public void registerBeanDefinition(String beanName, BeanDefinition beanDefinition)
		...
		// 获取当前存在的这个BeanDefinition(大部分为空),这是为了处理"bean覆盖",前面设置的allowBeanDefinitionOverriding
		BeanDefinition existingDefinition = this.beanDefinitionMap.get(beanName);


		// 处理重复名称的Bean定义的情况
		if (existingDefinition != null) {
			// 如若有相同名字的BeanDefinition,但又不允许覆盖,那么就会抛异常
			...
			// 判断是否已经有其他Bean开始初始化了
			// 注意,"注册Bean"结束以后,Bean依旧没有初始化
			// 在Spring容器启动时,会预初始化所有的Singleton Beans
			if (hasBeanCreationStarted()) {
				...
				synchronized (this.beanDefinitionMap) {
					...
					// 对beanDefinitionNames扩容+1
					// 并把当前beanName添加到BeanDefinitionNames中
					...
					if (this.manualSingletonNames.contains(beanName)) {
						...
						// 把这个beanName从manualSingletonNames删除
				...
				// 注册beanDefinition,beanDefinitionMap中保存了所有的BeanDefinition
				this.beanDefinitionMap.put(beanName, beanDefinition);

				// 这是个ArrayList.记录beanName
				this.beanDefinitionNames.add(beanName);

				// 从手动单例的记录列表中删除beaName
				// 这是个LinkedHashSet,代表手动注册的singleton bean(registerSinleton(String beanName,Object singletonObject))
